        ...
        public async void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            try
            {
                ...
                if (jwtToken is JwtSecurityToken)
                {
                    ...
                    if (!result)
                    {
                        context.Result = new BadRequestObjectResult(new { Status = 400, Message = "Sai định dạng token" });
                        return;
                    }
                }
                // 2.Check token đã hết hạn chưa
            ...
            {
                context.Result = new UnauthorizedObjectResult(new { Status = 401, Message = ex.Message });
                return;
            }
